...
-- written 'started' (with a search_trials MAX(id) watermark + the canonical grid hash) BEFORE the
-- compute and flipped 'completed' AFTER both rows landed. A crash mid-compute leaves 'started':
-- the re-run dedups the trial layer on (strategy_name, grid_json, id > watermark) — duplicate
-- trials are NOT harmless (they permanently inflate funnel/window breadth and the agent-NOVEL
-- lifetime seed) — and a 'completed' marker blocks any re-record. UNIQUE(strategy_id, branch_tip):
