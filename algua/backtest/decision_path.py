"""The canonical/fast dual decision path + its parity guard.

The fast vectorized path (`_fast_weights` / `_decision_weights_fast`) is only sound BECAUSE the
parity guard proves it agrees with the canonical per-bar loop (`_decision_weights` /
`_canonical_row`) — the sampling bound and the assertion message ARE the safety property. A
...
`engine.py` needs the dual-path selector from this module, so neither module can own `adj_grid`
without creating an import cycle with the other.
"""
