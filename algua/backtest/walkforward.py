...
def walk_forward(
    ...
    *,
    ...
    # Holdout STATISTIC + single-use burn: the holdout metric is evaluated on the NEXT line, so any
    # single-use burn the caller commits in on_peek is durable before the peek. Nothing
    # fallible-and-releasing may be added between on_peek and the holdout evaluation.
    if on_peek is not None:
        on_peek(cfg_hash)
    holdout_metrics: dict[str, Any] = _segment_record(returns, holdout[0], holdout[1])
