...
GATE PHILOSOPHY — market-first selection (the strategy-factory pivot, see
``docs/superpowers/specs/2026-08-10-strategy-factory-design.md``). Backtest evidence is cheap and
search can overfit it; forward evidence cannot be overfit by prior search. Selection therefore
happens at the FORWARD gate (the harsh threshold: ≥63 broker-clocked observations, realized Sharpe
≥ max(0.5×holdout, 0.3), vol/drawdown bounds), where multiplicity cannot reach. This gate is an
INTEGRITY FLOOR, not a statistical exam: only the correctness checks BIND —
``min_holdout_observations`` (the power floor), ``pit_required`` (Wall B), and
``holdout_sharpe_floor`` (raw holdout Sharpe > 0). The entire statistical stack — the
breadth-deflated ``holdout_sharpe`` bar, ``holdout_return``, window stability, DSR evidence, the
serial-dependence bootstrap, regime robustness, and the idiosyncratic-alpha screen — still
COMPUTES exactly as before and is RECORDED on every check (``"advisory": true``) but no longer
vetoes ``passed``. The recorded holdout Sharpe still parameterizes the forward bar (0.5×holdout),
so an overfit-inflated holdout self-punishes at the forward gate.

The pure-maths responsibilities that ``evaluate_gate`` composes live in cohesive sibling
...
class GateDecision:
    ...
    # LORD++ FDR accounting (#220, Phase 2; recalibrated #529; ADVISORY since the factory soft
    # gate). The promote path always records p_value=None (fdr_skip_reason="stats_advisory"), so
    # the store writes fdr_binding=NULL rows: the LORD++ stream is never advanced and no fdr_*
    # field below is populated. The fields and the ledger machinery are PRESERVED for future
    # re-tightening; fdr_skip_reason explains why FDR was omitted.
    ...
    # Serial-dependence bootstrap audit (#221 Slice 2). ARMED/EVALUATED (the check is computed +
    # appended, advisory — veto power removed) only when dsr_binding AND bootstrap_binding are both
    # True (gates computes effective_bootstrap_binding = dsr_binding AND bootstrap_binding). The
    # caller (promotion.py) sets bootstrap_binding only when measured breadth AND the OOS vector is
    # present. Not armed: all dsr_bootstrap_* fields remain False/None (omit-not-fail).
    ...
    # Multi-regime robustness audit (#221 Slice 4). Populated by evaluate_gate when the check is
    # ARMED/EVALUATED (holdout_returns + market_returns present, overlap >=
    # MIN_REGIME_OVERLAP_BARS) — the appended check is advisory (veto power removed).
    ...
    regime_robustness_binding: bool = False   # True iff the check was ARMED/EVALUATED (appended)
    # Market-beta / idiosyncratic-alpha screen (#328). Populated by evaluate_gate when the check is
    # ARMED/EVALUATED (holdout_returns + market_returns present, overlap >= IR_MIN_OVERLAP_BARS) —
    # the appended check is advisory (veto power removed). When omitted (unavailable / insufficient
    # overlap): ir_method explains why and ir_binding is False. market_beta is recorded for AUDIT
    # ONLY — there is no hard beta cap (it would over-reject legitimate high-alpha/high-beta
    # strategies); the check scores the appraisal ratio.
...
class GateSpec:
    ...
    GateCriteria field; `op` is the comparison operator. `advisory` checks are computed and
    recorded exactly like binding ones but carry ``"advisory": true`` and never veto ``passed``
    (the factory soft gate — only the integrity floor binds).
    """
...
GATE_SPECS: tuple[GateSpec, ...] = (
    GateSpec("holdout_sharpe", "holdout", "sharpe", "min_holdout_sharpe", ">=", advisory=True),
    ...
    GateSpec("pct_positive_windows", "stability", "pct_positive_windows",
             "min_pct_positive_windows", ">=", advisory=True),
...
def evaluate_gate(
    ...
    *,
    ...
    Factory soft gate: ``passed`` composes ONLY the binding integrity-floor checks
    (``min_holdout_observations``, ``holdout_sharpe_floor``, ``pit_required``). Every statistical
    check is computed and recorded exactly as before, marked ``"advisory": true``, and never
    vetoes. The holdout-Sharpe threshold of the (advisory) ``holdout_sharpe`` check is still
    DEFLATED by ``sharpe_haircut(n_combos, T)`` where T is the holdout sample size
    (``wf.holdout_metrics["n_bars"]``) — the multiple-testing telemetry: the more combos the agent
    searched, the higher the recorded bar. At N=1 (or no breadth) the haircut is 0 and the
    effective bar equals the base. ``breadth_provenance`` ("measured"/"declared") is carried into
    the decision for the audit trail; it does not change the math. The ``holdout_sharpe`` check's
    VALUE is read by name by the forward gate (``qualified_holdout_sharpe``) to parameterize the
    forward bar — its name/value/threshold semantics are a stable contract.
    """
    ...
    for spec in GATE_SPECS:
        ...
        if not math.isfinite(threshold):
            check: dict[str, Any] = {"name": spec.name, "value": None, "threshold": None,
        ...
        # gate failure, never a pass — and is never recorded as a NaN/inf in the payload.
            ...
            check = {"name": spec.name, "value": None, "threshold": threshold,
                     "op": spec.op, "passed": False}
        else:
            check = {"name": spec.name, "value": value, "threshold": threshold,
                     "op": spec.op, "passed": bool(_OPS[spec.op](value, threshold))}
        if spec.advisory:
            check["advisory"] = True
        checks.append(check)
    # Integrity floor (factory soft gate): the RAW holdout Sharpe must be strictly positive — a
    # correctness screen (a strategy that lost money out-of-sample has no business in the paper
    # book), NOT a statistical bar (the breadth-deflated holdout_sharpe check above is advisory).
    # Same value the holdout_sharpe check scores; fixed threshold 0.0, no criteria knob, no actor
    # split. A non-finite value FAILS CLOSED and is nulled out of the payload (same convention as
    # the spec loop above).
    raw_holdout_sharpe = float(wf.holdout_metrics["sharpe"])
    if not math.isfinite(raw_holdout_sharpe):
        checks.append({"name": "holdout_sharpe_floor", "value": None, "threshold": 0.0,
                       "op": ">", "passed": False})
    else:
        checks.append({"name": "holdout_sharpe_floor", "value": raw_holdout_sharpe,
                       "threshold": 0.0, "op": ">",
                       "passed": bool(raw_holdout_sharpe > 0.0)})
    ...
    # DSR evidence (#211): an ADVISORY check, appended ONLY when armed (measured trial dispersion
    # is available). When not armed it is omitted entirely (omit-not-fail).
    ...
    if dsr_binding:
        ...
        checks.append({"name": "dsr_evidence", "value": dsr_value,
                       ...
                       "advisory": True})
        if dsr_conf is None:
            # measured sweep exists but stats missing -> recorded as a failed advisory check
    ...
    # Multi-regime robustness (#221 Slice 4): an ADVISORY check, appended ONLY when the
    ...
    # Arming is INDEPENDENT of dsr_binding — purely a holdout-robustness audit.
    ...
    if holdout_ret_vec is None or market_returns is None:
        ...
        if overlap_n < MIN_REGIME_OVERLAP_BARS:
            ...
            checks.append({"name": "regime_robustness", "value": None,
                           ...
                           "passed": bool(res.passed), "advisory": True})
    ...
    # Market-beta / idiosyncratic-alpha screen (#328): an ADVISORY check appended ONLY when
    ...
    # regime sibling. Armed-but-degenerate (constant mkt, zero residual, non-finite) records a
    # failed advisory check. Arming is INDEPENDENT of dsr_binding and regime arming — purely a
    # beta-attribution screen.
    ...
    if holdout_ret_vec is None or market_returns is None:
        ...
        if ir.overlap_n < IR_MIN_OVERLAP_BARS:
            ...
            checks.append({"name": "idiosyncratic_alpha", "value": ir_value,
                           ...
                           "passed": bool(ir_passed), "advisory": True})
    ...
    return GateDecision(
        # Factory soft gate: only BINDING checks (no "advisory" marker — the integrity floor:
        # min_holdout_observations, holdout_sharpe_floor, pit_required) compose `passed`.
        # Advisory checks are recorded verbatim but never veto.
