...
def test_v33_adds_fdr_algo_epoch_column(tmp_path):
    # #324: ADDIS epoch marker. Additive nullable column; legacy LORD++ rows stay NULL and are
    # excluded from the ADDIS stream.
    conn = connect(tmp_path / "r.db")
    migrate(conn)
    cols = {r["name"] for r in conn.execute("PRAGMA table_info(gate_evaluations)")}
    assert "fdr_algo" in cols
