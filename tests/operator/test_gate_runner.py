...
and operator environment can never leak into the suite. A green gate returns the gated TREE SHA
(the ``write-tree`` snapshot it ran on) so the saga can bind ``commit_merge`` to it; a red gate is
``None``. Commands are injected (the production default is the real 4-command gate) so these tests
prove the hermetic properties without running an 8-minute pytest inside pytest."""
...
def _is_tree_sha(value: object) -> bool:
...
def test_gates_staged_tree_not_ambient_files(repo: Path) -> None:
    ...
    result = run_hermetic_quality_gate(
        repo,
        commands=[["test", "-f", "staged_marker.txt"], ["test", "!", "-f", ".env"]],
    )
    # Green returns the gated tree sha — exactly the write-tree of the staged index.
    assert _is_tree_sha(result)
    assert result == _git(repo, "write-tree")
...
def test_operator_env_is_scrubbed(repo: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("PYTHONPATH", "/live/checkout")
    ...
    result = run_hermetic_quality_gate(
        ...
        commands=[[
            ...
            ' && test -z "$PYTHONPATH" && test -z "$PYTEST_ADDOPTS"',
        ]],
    )
    assert _is_tree_sha(result)
...
def test_gate_child_stdout_routed_to_stderr(
    repo: Path, capfd: pytest.CaptureFixture[str],
) -> None:
    # The parent CLI's stdout is a JSON contract: gate children must not pollute it. Their stdout
    # is routed to the parent's STDERR (fd 2) — still visible in journald, never in the JSON.
    result = run_hermetic_quality_gate(
        repo, commands=[["sh", "-c", "echo GATE-NOISE-ON-STDOUT"]])
    assert _is_tree_sha(result)
    out, err = capfd.readouterr()
    assert "GATE-NOISE-ON-STDOUT" not in out
    assert "GATE-NOISE-ON-STDOUT" in err


def test_red_command_fails_closed_and_cleans_up(repo: Path) -> None:
    baseline = _worktree_count(repo)
    assert run_hermetic_quality_gate(repo, commands=[["false"]]) is None
...
def test_green_cleans_up_worktree(repo: Path) -> None:
    baseline = _worktree_count(repo)
    assert _is_tree_sha(run_hermetic_quality_gate(repo, commands=[["true"]]))
    assert _worktree_count(repo) == baseline


def test_plumbing_failure_is_red_not_raise(tmp_path: Path) -> None:
    # Not a git repo at all: the gate must return None, never raise into the saga.
    not_repo = tmp_path / "not_a_repo"
    not_repo.mkdir()
    assert run_hermetic_quality_gate(not_repo, commands=[["true"]]) is None
