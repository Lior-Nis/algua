...
**Ex-date data source (the seam).** The accrual needs an ex-date-timed dividend **declaration** `(symbol, corp_action_id, ex_date, gross_per_share, payment_date)` — gross from corporate-action metadata (Decision 0), never net account cash. The live/execution lane is import-walled from `algua.data`, so the source is the **broker adapter**, not the research corpactions module:

- **Live:** the accrual's authoritative input is Alpaca's **corporate-action / dividend-declaration metadata** — the record that states the stable `corp_action_id`, the **gross (pre-withholding) cash per share**, the `ex_date`, and the `payment_date`. (The payment-date DIV account activity carries only **net** cash and is used solely for reconciliation — §2 / Decision 0.) The activity ingestion (`_ingest_one_activity`) already parses non-fill activities; we extend it to recognize a whitelisted DIV, resolve its declaration record, and drive the accrual from the declared gross. **Fail-closed on any missing declared input:** a DIV whose `gross_per_share`, `ex_date`, or `corp_action_id` is unresolvable from corporate-action metadata is routed to suspense — we never guess `ex_date = payment_date` (reintroduces the timing break) and never derive `gross_per_share` from net account cash (reintroduces the withholding break).
...
5. **Accrual write path — live ingestion (`live_ledger._ingest_one_activity` / `ingest_activities`).** On a whitelisted DIV: resolve its declaration (`corp_action_id`, `gross_per_share`, `ex_date`, `payment_date`) from corporate-action metadata, compute `entitlement_cutoff` (task 3), derive per-strategy accruals (task 3), and **correction-aware-upsert** (§2 `ON CONFLICT DO UPDATE ... WHERE status='accrued'`, NOT `INSERT OR IGNORE`) into `dividend_accruals`, respecting the prospective `cutover_ts` (task 8). A DIV with no resolvable declared gross/ex_date/corp_action_id → suspense (fail-closed). Idempotent on window re-pull; a corrected open accrual self-heals. Tests: accrual rows created once, ex-date-timed, cutover-gated; no-declared-gross → suspense; correction updates an open row; identical re-pull is a no-op.
...
7. **Payment-date reconciliation.** On the payment-date DIV cash activity, match the open `accrued` rows **by `corp_action_id`** (the stable key — NOT a lane/symbol/ex-date window; a fuzzy window is a fallback only if the broker omits the id, and an unmatched payment surfaces a discrepancy rather than guessing), set `status='paid'`, record `reconciled_cash` (attributed share of net cash), compute the withholding residual and the orphan (account-vs-ledger qty) residual — WITHOUT re-crediting NAV. A post-payment corrected declaration is a restatement discrepancy (task 2 upsert guard), not a silent NAV edit. Tests: payment does not change NAV; matched by `corp_action_id`; status/residual recorded; residual surfaced not deducted; unmatched-payment and post-payment-restatement discrepancies surfaced.
8. **Prospective rollout + basis stamp + cross-basis gate.** Persist per-strategy `cutover_ts` on first tick under the new basis; accrual (task 5/6) filters `ex_date >= cutover_ts`. Stamp `accounting_basis_version` on the NAV snapshot / forward certificate. **Enforce the §6 basis-equality precondition:** the forward/promotion gate rejects (fail-closed) any certificate or evidence window whose `accounting_basis_version` is older than the strategy's current live basis, or that straddles the cutover — pre-cutover / mixed-basis evidence can NEVER satisfy a post-cutover comparison. **First determine (spike) whether this can be enforced by reading the stamp in a non-protected read path or requires editing `forward_gates.py`/`forward_promotion.py`/`live_gate.py` (CODEOWNERS — then the PR stays OPEN for human merge).** Tests: pre-cutover ex-date not accrued; post-cutover accrued; certificate carries the basis version; an old-basis and a straddling evidence window are both rejected by the gate; no peak-table migration required.
